...
def overlay_masks(img, detections):
    ...
    for det in detections:
        ...
        for c in range(3):  # R, G, B
            result[:, :, c] = np.where(
                mask, result[:, :, c] * (1 - alpha) + color[c] * alpha, result[:, :, c]
            )
        ...
        try:
            ...
            for det in detections:
                ...
                if shape_info:
                    ...
                    stats_text += (
                        f"• {name} ({conf*100:.1f}%): не удалось определить форму\n"
                    )
            ...
            for det in detections:
                ...
                if media:
                    try:
                        ...
                        print(f"Ошибка отправки медиа группы сегментации: {e}")
                ...
                if name == "person":
                    ...
                    if pose_buf:
                        ...
                        media.append(
                            InputMediaPhoto(
                                media=pose_buf,
                                caption="Современный (YOLOv8-pose)"
                            )
                        )
                else:
                    ...
                    if yolo_skel_buf:
                        ...
                        media.append(
                            InputMediaPhoto(
                                media=yolo_skel_buf,
                                caption="Современный (YOLOv8-seg)"
                            )
                        )

                # Отправляем альбом (только если есть хотя бы одно фото)
                if media:
                    try:
                        bot.send_media_group(
                            message.chat.id,
                        ...
                        print(f"Ошибка отправки медиа группы скелетов: {e}")
        finally:
