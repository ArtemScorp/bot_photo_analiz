...
def segment_object(img, box_points):
    """Вырезает объект по bbox и возвращает бинарную маску контуров"""
    ...
    # Бинаризация Otsu — чёткие контуры без шума
...
def overlay_contours(img, detections):
    """Накладывает контуры всех объектов на оригинальное изображение"""
    result = img.copy()
    ...
    for det in detections:
        ...
        box = det["box_points"]
        x1, y1, x2, y2 = box
        draw.text((x1, max(0, y1 - 10)), name, fill=(255, 0, 0))
...
def handle_photo(message):
    try:
        ...
        # Загружаем оригинал и накладываем контуры объектов
        ...
        # Отправляем отдельные сегментированные объекты
        if os.path.exists(extracted_dir):
